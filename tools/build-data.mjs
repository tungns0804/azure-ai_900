 ...
 *
 *   data/extra-*.mjs      → public/ai900-extra.json  (câu hỏi BỔ SUNG)
 *   data/vi-original-*.mjs
 *   + phần vq/vo/ve của extra-*.mjs
 *                         → public/ai900-vi.json     (bản dịch tiếng Việt kèm sẵn)
 *
...
/* bản dịch cho 115 câu gốc */
const original = JSON.parse(readFileSync(join(publicDir, 'ai900-data.json'), 'utf8'));
...
for (const { file, value } of await loadAll('vi-original-')) {
  for (const [key, src] of Object.entries(value)) {
    ...
    if (!q) {
      fail(`${file}: câu ${id} không có trong ai900-data.json`);
      continue;
    }
...
writeFileSync(join(publicDir, 'ai900-extra.json'), JSON.stringify(questions), 'utf8');
writeFileSync(join(publicDir, 'ai900-vi.json'), JSON.stringify(vi), 'utf8');
...
console.log(`ai900-extra.json : ${questions.length} cau bo sung`);
...
console.log(`ai900-vi.json    : ${Object.keys(vi).length} khoa dich`);
